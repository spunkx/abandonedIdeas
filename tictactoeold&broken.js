...
function runtictactoe(x,y){
    ...
    gridInteractions(xy, maxlineSize);
    //console.log(grid);
    drawnState = 1;
...
function gridInteractions(xy, maxlineSize){
    var redCounter = 0;
    var greenCounter = 0;
    var blueCounter = 0;
    canvasWrap.addEventListener('mouseout', (e) => {
        //e is the event object
        interact(e, xy, 0, redCounter, greenCounter, blueCounter);
    })
    ...
    canvasWrap.addEventListener('click', (e) => {
        interact(e, xy, maxlineSize);
    })
}

function interact(event, maxlineSize, r, g, b){
    ...
    if(event.type == "mouseover"){
        let centreDiv = boxy.querySelector("." + event.target.className);
        //console.log(event.target.style.left+event.target.style.width);
    ...
    if(event.type == "click"){
        //let selectorString = ("." + event.target.className);
        let centreDiv = boxy.querySelector("." + event.target.className);
        

        //drawLine(event.target.style.left+event.target.style.width+10, event.target.style.width-10, gameSelector, "posdiag");
        //drawLine(event.target.style.left+10, event.target.style.width-10, gameSelector, "negdiag");
